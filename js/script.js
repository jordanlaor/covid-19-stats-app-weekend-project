...
function handleError(error) {
  ...
  setTimeout(() => (errorWrapper.textContent = ''), 8000);
}
...
async function countryClick(e, continent) {
  // TODO convert to const
  try {
    covidData = await fetchData(`${urls.covid}/${world[continent][e.target.textContent].code}`);
    const labels = [];
    const dataConfirmed = [];
  ...
  try {
    ...
    if (e.target.textContent === 'World') {
      Object.keys(world).forEach((continent) => {
        ...
        btnsCountry.addEventListener('click', (ev) => countryClick(ev, continent));
      });
    } else {
      ...
      btnsCountry.addEventListener('click', (ev) => countryClick(ev, e.target.textContent));
    }
